class SportsMania{

static String iplTeams[] = {"CSK","RCB","MI","PBKS","DC","LSG","SRH","KKR","RR","GT"};
static String top10BatsMen[] = {"MSD","VK","RP","SG","KLR","RS","HP","DW","SS","DJB"};
static String top10Bowlers[] = {"DJB","BK","JB","MSh","MS","TS","N","G","P","AK"};
static String top10ICCTeamsName[] = {"India","Australia","South Africa","Bangladesh","New Zealand","England","West Indies","Ireland","Zimbawe","Sri Lanka"};
	public static void main(String[]sportsInfo){
	System.out.println("The list of IPL Teams are: \n " + iplTeams[0]+"\n"+iplTeams[1]+"\n"+iplTeams[2]+"\n"+iplTeams[3]+"\n"+iplTeams[4]+"\n"+iplTeams[5]+"\n"+iplTeams[6]+"\n"+iplTeams[7]+"\n"+iplTeams[8]+"\n"+iplTeams[9]+"\n");
	System.out.println("The list of Top 10 BatsMen are: \n " + top10BatsMen[0]+"\n"+top10BatsMen[1]+"\n"+top10BatsMen[2]+"\n"+top10BatsMen[3]+"\n"+top10BatsMen[4]+"\n"+top10BatsMen[5]+"\n"+top10BatsMen[6]+"\n"+top10BatsMen[7]+"\n"+top10BatsMen[8]+"\n"+top10BatsMen[9]+"\n");
	System.out.println("The list of Top 10 Bowlers are: \n " + top10Bowlers[0]+"\n"+top10Bowlers[1]+"\n"+top10Bowlers[2]+"\n"+top10Bowlers[3]+"\n"+top10Bowlers[4]+"\n"+top10Bowlers[5]+"\n"+top10Bowlers[6]+"\n"+top10Bowlers[7]+"\n"+top10Bowlers[8]+"\n"+top10Bowlers[9]+"\n");
	System.out.println("The list of Top 10 ICC Teams are: \n " + top10ICCTeamsName[0]+"\n"+top10ICCTeamsName[1]+"\n"+top10ICCTeamsName[2]+"\n"+top10ICCTeamsName[3]+"\n"+top10ICCTeamsName[4]+"\n"+top10ICCTeamsName[5]+"\n"+top10ICCTeamsName[6]+"\n"+top10ICCTeamsName[7]+"\n"+top10ICCTeamsName[8]+"\n"+top10ICCTeamsName[9]+"\n");





	}
}